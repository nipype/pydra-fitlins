...
ModelSpecLoader_input_fields = [
    (
        "database_path",
        Directory,
        {
            "help_string": "Path to bids database",
        },
    ),
    (
        "model",
    ...
    (
        ...
        dict,
        {
            "help_string": "Limit models to those with matching inputs",
        },
    )
]

ModelSpecLoader_input_spec = SpecInfo(
    name="ModelSpecLoaderInputSpec",
    fields=ModelSpecLoader_input_fields,
    bases=(BaseSpec,),
)

ModelSpecLoader_output_fields = [
    (
        "model_spec",
        dict,
        {
...
ModelSpecLoader_output_spec = SpecInfo(
    name="ModelSpecLoaderOutputSpec",
    fields=ModelSpecLoader_output_fields,
    bases=(BaseSpec,),
)



class ModelSpecLoader(FunctionTask):
    """
    Load BIDS Stats Models specifications from a BIDS directory
    """

    input_spec = ModelSpecLoader_input_spec
    output_spec = ModelSpecLoader_output_spec
...
LoadBIDSModel_input_fields = [
    ...
    (
        ...
        dict,
        {
            "help_string": "Model specification",
            "mandatory": True,
        },
    ),
    (
        ...
        dict,
        {
            "help_string": "Limit collected sessions",
        },
    )
]

LoadBIDSModel_input_spec = SpecInfo(
    name="LoadBIDSModelInputSpec",
    fields=LoadBIDSModel_input_fields,
    bases=(BaseSpec,),
)

LoadBIDSModel_output_fields = [
    (
        "design_info",
        list,
        {
            "help_string": "Descriptions of design matrices with sparse events, dense regressors and TR",
        },
    ),
    (
        "warnings",
        list,
        {
            "help_string": "HTML warning snippet for reporting issues",
        },
    ),
    (
        "all_specs",
        dict,
        {
            "help_string": "A collection of all specs built from the statsmodel"
        }
    )
]

LoadBIDSModel_output_spec = SpecInfo(
    name="LoadBIDSModelOutputSpec",
    fields=LoadBIDSModel_output_fields,
    bases=(BaseSpec,),
)


class LoadBIDSModel(FunctionTask):
    """
    ...
    input_spec = LoadBIDSModel_input_spec
    output_spec = LoadBIDSModel_output_spec
...
BIDSSelect_input_fields = [
    ...
    (
        ...
        dict,
        {
            "help_string": "Additional selectors to be applied",
        },
    )
]

BIDSSelect_input_spec = SpecInfo(
    name="BIDSSelectInputSpec",
    fields=BIDSSelect_input_fields,
    bases=(BaseSpec,),
)

BIDSSelect_output_fields = [
    (
        "bold_files",
        MultiOutputFile,
        {
            "help_string": "bold files",
        },
    ),
    (
        "mask_files",
...
BIDSSelect_output_spec = SpecInfo(
    name="BIDSSelectOutputSpec",
    fields=BIDSSelect_output_fields,
    bases=(BaseSpec,),
)



class BIDSSelect(FunctionTask):
    input_spec = BIDSSelect_input_spec
    output_spec = BIDSSelect_output_spec
...
BIDSDataSink_input_fields = [
    (
        "base_directory",
        Directory,
        {
            "help_string": "Path to BIDS (or derivatives) root directory",
            "mandatory": True,
        },
    ),
    (
    ...
    (
        ...
        MultiInputFile,
        {
            "help_string": "Per-file entities to include in filename",
        },
    ),
    (
        "fixed_entities",
        dict,
        {
            "help_string": "Entities to include in all filenames",
        },
    ),
    (
        "path_patterns",
        MultiInputFile,
        {
            "help_string": "BIDS path patterns describing format of file names",
        },
    )
]

BIDSDataSink_input_spec = SpecInfo(
    name="BIDSDataSinkInputSpec",
    fields=BIDSDataSink_input_fields,
    bases=(BaseSpec,),
)

BIDSDataSink_output_fields = []

BIDSDataSink_output_spec = SpecInfo(
    name="BIDSDataSinkOutputSpec",
    fields=BIDSDataSink_output_fields,
    bases=(BaseSpec,),
)


class BIDSDataSink(IOBase):
    input_spec = BIDSDataSink_input_spec
    output_spec = BIDSDataSink_output_spec
